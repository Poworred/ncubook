...
| 组件文件 | 组件名称 | 核心职责与交互特性 |
| :--- | :--- | :--- |
| **`header.tsx`** | `AppHeader` | 全局统一移动端顶部导航栏。**文档页**：左目录（`☰`）、中当前文档标题、右搜索（`Q`），下沿独立显示阅读进度条；**首页**：左品牌、右搜索+目录；严格保障 44px 舒适热区。 |
...
| **`entry.tsx`** | `AskEntry` | 全局问答入口。**首页**：胶囊提问框（白底浅灰线，右侧吉祥物触发器）；**文档页**：**52px 完整正圆固定吉祥物按钮**（`right: 16px; bottom: 18px;` 脱离正文滚动流），点击自动感知当前篇章上下文。 |
